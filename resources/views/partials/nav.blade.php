            ...
            @if(count($menu->children))
                ...
                <div @class(['floatingMenu','has_oneColumn'=>$menuColumns])">
                    <div class="row">
                        ...
                        <div @class(['col-lg-12'=>$menuColumns,'col-lg-6 col-md-12'=>!$menuColumns])="col-lg-6 col-md-12">
                            <div class="floatingMenu_col">

                                <h6>
                                    <span><img src="/bizionic/images/{{$cmenu->data->menu_icon}}" alt="{{$cmenu->data->menu_icon}}"/></span>
                                    {{ $cmenu->name }}
                                ...
                                <div class="floatingMenu_col_list ">
                                    <ul>
                                        @foreach($cmenu->children as $cItemkey => $cMenuItem)
                                            <li><a href="{{url("/{$menu->data->slug}/{$cmenu->data->slug}/{$cMenuItem->data->slug}")}}" class="">{{$cMenuItem->name}}</a></li>
                                        @endforeach
                                    </ul> 
                                </div>
                                @endif
